...
with open(cancervar_file, 'r') as tsv_file:
	...
	for vep_values in cancervar_handle:
		...
		cancervar_id = str(chrom) + ':' + str(start) + ':' + str(end) + ':' + str(ref) + ':' + str(alt)
...
with open(vep_file, 'r') as vcf:
	...
	writer.writerow([*header, "CancerVar evidence", "AI score"])
	...
	for str_lines in csv_handle:
		...
		vep_id = str(chrom_vep) + ':' + str(start_vep) + ':' + str(end_vep) + ':' + str(ref_vep) + ':' + str(alt_vep)

		if vep_id in map_evidence:
			evidence_val = map_evidence[vep_id]
			ensembl_score_val = map_ensembl_score[vep_id]

		writer.writerow([*str_lines, evidence_val, ensembl_score_val])
