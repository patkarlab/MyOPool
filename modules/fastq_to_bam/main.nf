...
process ALIGNMENT_METRICS_WGS {
	label 'process_low'
	publishDir "${params.outdir}/${Sample}/${Sample}-WGS/", mode: 'copy', pattern: '*_alignment_summary_metrics.txt'
	tag "${Sample}"
	input:
		tuple val(Sample), file(final_bam), file(final_bam_bai)
		path (GenFile)
...
process INSERT_SIZE_METRICS_WGS {
	label 'process_low'
	publishDir "${params.outdir}/${Sample}/${Sample}-WGS/", mode: 'copy', pattern: '*_insert_size_metrics.txt'
	publishDir "${params.outdir}/${Sample}/${Sample}-WGS/", mode: 'copy', pattern: '*_insert_size_metrics.pdf'
	tag "${Sample}"
	input:
		tuple val(Sample), file(final_bam), file(final_bam_bai)
	output:
